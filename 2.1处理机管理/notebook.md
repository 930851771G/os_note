...
* gxy总结：
  ...
  掌握再引入线程之后，一些很重要的变化，进程是资源分配的单位，线程是CPU调度的单位。
...
* 为什么要引入线程：(理解即可)
  ...
  **线程是一个基本的CPU执行单元，也是程序执行流的最小单位。**
  ...
  意思是计算机的资源，是分配给一个进程的，而不是分配给一个现成的。
...
* 引入线程之后带来的变化：
  ...
  * 并发性：
    之前只能进程间并发。
    引入线程之后，各个线程之间也可以并发，提升了并发度。
  * 系统开销方面：
    传统的进程间开发，需要经常切换进程的开发环境，系统开销比较大。
    引入线程之后，**如果是同一个进程里面的线程切换**，不需要切换进程的开发环境，开销很小。引入线程之后，并发所带来的系统开销减小。
...
* 线程实现方式
  ...
  * 用户级线程(早期的OS只支持进程，不支持线程，当时线程是通过线程库实现的)
    ...
    OS其实只看得到进程，进程再CPU上运行的时候，是程序员自己通过线程库创建了逻辑上的线程，也就是`用户级线程`。

    1.是谁管理?
    这些`用户级线程`的工作由`应用程序负责`。包括线程的切换。
    2.需要核心态吗？
    用户级线程的切换在`用户态下面就可以完成`，不需要OS系统干预。
    ...
    Q：当一个`用户级线程`阻塞的时候，整个进程都会被阻塞，并发度不高。多个线程不能在多核处理机上面运行。因为对于OS只看到了一个进程，所以是不会分配更多的核心的。
  ...
  * 内核级线程：
    ...
    2.线程切换是否需要CPU切换核心态？
    一位内线程调度、却换的工作都是内核在负责，因此`内核级线程的切换`必须要在`核心态`下完成。
...
<img src="E:/os/线程/img/image-20240514010030775.png" alt="image-20240514010030775" style="zoom:50%;" />
